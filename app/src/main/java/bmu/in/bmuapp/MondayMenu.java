...
public class MondayMenu extends AppCompatActivity {

    private ProgressDialog pDialog;

    HashMap<String, String> mondaymenuHashMap;        //new

    // JSON Node names
    private static final String TAG_MONDAY = "mondaymenu";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_monday_menu);
    ...
    private class GetContacts extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPreExecute() {
            ...
            pDialog = new ProgressDialog(MondayMenu.this);
        ...
        protected Boolean doInBackground(Void... arg0) {
            ...
            String url = "http://dcafe-menu.getsandbox.com/dcafe-menu-monday";
            ...
            if (jsonStr != null) {
                ...
                try {
                    ...
                    mondaymenu = jsonObj.getJSONArray(TAG_MONDAY);
                    Log.d("check 12 : ", "" +mondaymenu  );

                    mondaymenuHashMap = new HashMap<String, String>();
                    // looping through All Contacts
                    for (int i = 0; i < mondaymenu.length(); i++) {
                        JSONObject c = mondaymenu.getJSONObject(i);
                        ...
                        {
                            mondaymenuHashMap.put(TAG_DRY_CEREAL, DRY_CEREAL);
                        ...
                        {
                            mondaymenuHashMap.put(TAG_DRINK_ONE, DRINK_ONE);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        //mondaymenu.put(TAG_DRINK_TWO, DRINK_TWO);
                        if (DRINK_TWO!=null&&!DRINK_TWO.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_DRINK_TWO, DRINK_TWO);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_MAIN, MAIN);
                        if (MAIN!=null&&!MAIN.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_MAIN, MAIN);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }
                        // mondaymenu.put(TAG_ACCOMPANIMENT, ACCOMPANIMENT);
                        if (ACCOMPANIMENT!=null&&!ACCOMPANIMENT.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_ACCOMPANIMENT, ACCOMPANIMENT);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        // mondaymenu.put(TAG_B_BREAD, B_BREAD);
                        if (B_BREAD!=null&&!B_BREAD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_B_BREAD, B_BREAD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        // mondaymenu.put(TAG_L_SALAD, L_SALAD);
                        if (L_SALAD!=null&&!L_SALAD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_SALAD, L_SALAD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        //  mondaymenu.put(TAG_L_RICE, L_RICE);
                        if (L_RICE!=null&&!L_RICE.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_RICE, L_RICE);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_L_DAL, L_DAL);

                        if (L_DAL!=null&&!L_DAL.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_DAL, L_DAL);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_L_PANEER, L_PANEER);

                        if (L_PANEER!=null&&!L_PANEER.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_PANEER, L_PANEER);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        //mondaymenu.put(TAG_L_SEMI, L_SEMI);

                        if (L_SEMI!=null&&!L_SEMI.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_SEMI, L_SEMI);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        //mondaymenu.put(TAG_L_GRAVY, L_GRAVY);

                        if (L_GRAVY!=null&&!L_GRAVY.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_GRAVY, L_GRAVY);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        //mondaymenu.put(TAG_L_DESSERT, L_DESSERT);

                        if (L_DESSERT!=null&&!L_DESSERT.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_DESSERT, L_DESSERT);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        // mondaymenu.put(TAG_L_BREAD, L_BREAD);

                        if (L_BREAD!=null&&!L_BREAD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_BREAD, L_BREAD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        //mondaymenu.put(TAG_L_CURD, L_CURD);

                        if (L_CURD!=null&&!L_CURD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_L_CURD, L_CURD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        //mondaymenu.put(TAG_SNACKS, SNACKS);

                        if (SNACKS!=null&&!SNACKS.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_SNACKS, SNACKS);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_DRINK, DRINK);

                        if (DRINK!=null&&!DRINK.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_DRINK, DRINK);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        //mondaymenu.put(TAG_D_SALAD, D_SALAD);

                        if (D_SALAD!=null&&!D_SALAD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_SALAD, D_SALAD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_D_RICE, D_RICE);

                        if (D_RICE!=null&&!D_RICE.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_RICE, D_RICE);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_D_DAL, D_DAL);
                        if (D_DAL!=null&&!D_DAL.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_DAL, D_DAL);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        //mondaymenu.put(TAG_D_PANEER, D_PANEER);
                        if (D_PANEER!=null&&!D_PANEER.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_PANEER, D_PANEER);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        //mondaymenu.put(TAG_D_SEMI, D_SEMI);

                        if (D_SEMI!=null&&!D_SEMI.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_SEMI, D_SEMI);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }


                        //mondaymenu.put(TAG_D_GRAVY, D_GRAVY);

                        if (D_GRAVY!=null&&!D_GRAVY.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_GRAVY, D_GRAVY);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }
                        // mondaymenu.put(TAG_D_DESSERT, D_DESSERT);

                        if (D_DESSERT!=null&&!D_DESSERT.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_DESSERT, D_DESSERT);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }

                        // mondaymenu.put(TAG_D_BREAD, D_BREAD);

                        if (D_BREAD!=null&&!D_BREAD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_BREAD, D_BREAD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }
                        // mondaymenu.put(TAG_D_CURD, D_CURD);

                        if (D_CURD!=null&&!D_CURD.isEmpty())

                        {
                            mondaymenuHashMap.put(TAG_D_CURD, D_CURD);
                            //Log.d("check  : ", "" +TAG_DRY_CEREAL  );
                        }
        ...
        protected void onPostExecute(Boolean aVoid) {
            ...
            dryCereal.setText(mondaymenuHashMap.get(TAG_DRY_CEREAL));
            drinkOne.setText(mondaymenuHashMap.get(TAG_DRINK_ONE));
            drinkTwo.setText(mondaymenuHashMap.get(TAG_DRINK_TWO));
            main.setText(mondaymenuHashMap.get(TAG_MAIN));
            accompaniment.setText(mondaymenuHashMap.get(TAG_ACCOMPANIMENT));
            bread.setText(mondaymenuHashMap.get(TAG_B_BREAD));

            lSalad.setText(mondaymenuHashMap.get(TAG_L_SALAD));
            lRice.setText(mondaymenuHashMap.get(TAG_L_RICE));
            lDal.setText(mondaymenuHashMap.get(TAG_L_DAL));
            lPaneer.setText(mondaymenuHashMap.get(TAG_L_PANEER));
            lSemi.setText(mondaymenuHashMap.get(TAG_L_SEMI));
            lGravy.setText(mondaymenuHashMap.get(TAG_L_GRAVY));
            lDessert.setText(mondaymenuHashMap.get(TAG_L_DESSERT));
            lBread.setText(mondaymenuHashMap.get(TAG_L_BREAD));
            lCurd.setText(mondaymenuHashMap.get(TAG_L_CURD));

            snacks.setText(mondaymenuHashMap.get(TAG_SNACKS));
            drink.setText(mondaymenuHashMap.get(TAG_DRINK));

            dSalad.setText(mondaymenuHashMap.get(TAG_D_SALAD));
            dRice.setText(mondaymenuHashMap.get(TAG_D_RICE));
            dDal.setText(mondaymenuHashMap.get(TAG_D_DAL));
            dPaneer.setText(mondaymenuHashMap.get(TAG_D_PANEER));
            dSemi.setText(mondaymenuHashMap.get(TAG_D_SEMI));
            dGravy.setText(mondaymenuHashMap.get(TAG_D_GRAVY));
            dDessert.setText(mondaymenuHashMap.get(TAG_D_DESSERT));
            dBread.setText(mondaymenuHashMap.get(TAG_D_BREAD));
            dCurd.setText(mondaymenuHashMap.get(TAG_D_CURD));
